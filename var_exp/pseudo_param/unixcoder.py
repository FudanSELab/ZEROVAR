...
class UniXcoderVarExplainer(AbstractVarExplainer):
    def __init__(self, device="cpu"):
        super(UniXcoderVarExplainer, self).__init__()
        self.device = torch.device(device)
        self.model = UniXcoder("microsoft/unixcoder-base")
        self.model.to(self.device)

    def generate_prompt_templates(self, method:str) -> list:
        tokens_ids = self.model.tokenize(["<mask0>\n" + method],max_length=512,mode="<encoder-decoder>")
        source_ids = torch.tensor(tokens_ids).to(self.device)
        prediction_ids = self.model.generate(source_ids, decoder_only=False, beam_size=3, max_length=128)
        predictions = self.model.decode(prediction_ids)
        prompt_template = [x.replace("<mask0>","").strip() for x in predictions[0]]
        # remove the meta character '%' in templates
        prompt_template = [x.replace("%","%%").strip() for x in prompt_template]
    ...
    def explain_for_var(self, method: str, prompt_templates: list, cand_num=10) -> dict:
        ...
        for template in prompt_templates:
            template = template.replace("[param explanation]", "<extra_id_0>")
            inpt = template + "\n" + method
            tokens_ids = self.model.tokenize([inpt],max_length=512,mode="<encoder-decoder>")
            source_ids = torch.tensor(tokens_ids).to(self.device)
            prediction_ids = self.model.generate(source_ids, decoder_only=False, beam_size=cand_num, max_length=128)
            predictions = self.model.decode(prediction_ids)
